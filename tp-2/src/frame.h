...
class Frame {
   ...
   private:
    ...
    void show_to(std::string const &filename, double *values) const {
        ...
        if (!f) {
            puts("can't write output image to disk!");
            return;
        }
        ...
        for (int i = height - 1; i >= 0; i--)  // bmp stores images upside down
        {
            for (int j = 0; j < width; j++) {
                ...
                mapped_intensity =
                    (unsigned char)((values[start + 2] - min_intensity) / (max_intensity - min_intensity) * 255);
                // mapped_intensity = (unsigned char)(max(0.0, min(1.0, color[start + 2] / 16)) * 255);
                fwrite(&mapped_intensity, 1, 1, f);

                mapped_intensity =
                    (unsigned char)((values[start + 1] - min_intensity) / (max_intensity - min_intensity) * 255);
                // mapped_intensity = (unsigned char)(max(0.0, min(1.0, color[start + 1] / 16)) * 255);
                fwrite(&mapped_intensity, 1, 1, f);

                mapped_intensity =
                    (unsigned char)((values[start] - min_intensity) / (max_intensity - min_intensity) * 255);
